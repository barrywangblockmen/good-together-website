...
ssh -i "$SSH_KEY_PATH" "${SSH_USER}@${SSH_HOST}" "
  ...
  if [[ -f .env.production ]]; then
    ...
    (crontab -l 2>/dev/null | grep -Fv \"\$CRON_MARKER\" | grep -Fv 'aitgp-fetch-prices' | grep -Fv '^CRON_TZ=Asia/Taipei$' || true; echo \"CRON_TZ=Asia/Taipei\"; echo \"\$CRON_MARKER\"; echo \"\$CRON_HOURLY\"; echo \"\$CRON_DAILY\") | crontab -
